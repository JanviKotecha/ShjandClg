...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['isChecked']) && isset($_POST['courseId'])) {
        ...
        $_SESSION['alert_msg'] .= "<div class='msg_success'>Toper active successfully.</div>";
        echo json_encode(['status' => 'success', 'message' => 'Toper active successfully']);
        exit;
    }
}
...
if (isset($_GET['id'])) {
    if ($_GET['id'] != '') {
        if (is_numeric($_GET['id'])) {
            $res = $qm->getRecord("topers", "*", "id=" . $_GET['id']);
            if (mysqli_num_rows($res) > 0) {
                $qm->deleteRecord("topers", "id=" . $_GET['id']);
                $result = mysqli_fetch_array($res);
                $_SESSION['alert_msg'] .= "<div class='msg_success'>Topers deleated successfully.</div>";
                ...
                <div class="content-wrapper px-4">
                    <div class="row page-title-header ">
                        <div class="col-lg-12 grid-margin stretch-card">
                            ...
                            <div class="card">
                                <div class="card-body">
                                    <h3 class="card-title">Topers Detail
                                        <a href="topers_add.php" class="btn btn-primary" style="float: right;"><i
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div class="avatar me-3">
                                                            <img src="<?php echo $row["img"]=='' ? TOPERS_URL.'noimg.png' : (file_exists(UPLOAD_TOPERS_URL.$row["img"]) ? TOPERS_URL.$row["img"] : TOPERS_URL.'noimg.png'); ?>"
                                                    ...
                                                    <td>
                                                        <a class="btn btn-primary width"
                                                            href="topers_update.php?id=<?php echo $row['id']; ?>"><i
                                                                class="fa fa-edit"></i></a>
                                                        <a class="btn btn-danger width"
                                                            href="topers.php?id=<?php echo $row['id']; ?>"
